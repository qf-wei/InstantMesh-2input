...
class InstantMesh(nn.Module):
    ...
    def get_sdf_deformation_prediction(self, planes):
        ...
        if torch.sum(zero_surface).item() > 0:
            ...
            max_sdf = sdf.max()
            min_sdf = sdf.min()
            update_sdf[:, self.geometry.center_indices] += (
                1.0 - min_sdf
            )  # greater than zero
    ...
    def forward_geometry(self, planes, render_cameras, render_size=256):
        ...
        mesh_v, mesh_f, sdf, deformation, v_deformed, sdf_reg_loss = (
            self.get_geometry_prediction(planes)
        )
        with torch.no_grad():
            ...
            tex_pos = [
                torch.cat(
                    [pos[i_view : i_view + 1] for i_view in range(run_n_view)], dim=2
                )
                for pos in tex_pos
            ]
